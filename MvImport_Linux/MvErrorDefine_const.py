...
MV_E_ENCRYPT                                 = 0x8000000E  # < 加密错误 | en:Encryption error
MV_E_OPENFILE                                = 0x8000000F  # < 打开文件出现错误 | en:open file error
MV_E_BUF_IN_USE                              = 0x80000010  # < 缓存地址已使用 | en:Buffer already in use
MV_E_BUF_INVALID                             = 0x80000011  # < 无效的缓存地址 | en:Buffer address invalid
MV_E_NOALIGN_BUF                             = 0x80000012  # < 缓存对齐异常 | en:Buffer alignmenterror error
MV_E_NOENOUGH_BUF_NUM                        = 0x80000013  # < 缓存个数不足 | en:Insufficient cache count
MV_E_PORT_IN_USE                             = 0x80000014  # < 串口被占用 | en:Port is in use
MV_E_IMAGE_DECODEC                           = 0x80000015  # < 解码错误(SDK校验图像异常) | en:Decoding error (SDK verification image exception)
